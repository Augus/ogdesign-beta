...
app.controller("ResourceController", function ($scope, $http, $timeout, $location, Analytics) {
	...
	$scope.categoryFilter = function (resource) {
        ...
        var categories = resource.categories.map(function (c) {
            return c._id;
        });
		...
		if (categories.indexOf($scope.currentCategory) != -1 ) {
			return true;
		}
		return false;
	};
